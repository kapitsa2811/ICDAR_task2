...
aug_rate=100
maxPrintLen = 18
pre_dir = '/home/sjhbxs/checkout/ICDAR_task2/ICDAR_TASK2_new26' 
    ...
    def __init__(self, data_dir,text_dir):
        ...
        for root, sub_folder, file_list in os.walk(data_dir):
            for file_path in file_list:
                    ...
                    if code_isin_charset(code) == False:
                        print("contain less char", code,num_from_file_path)
                        continue
                    code = [SPACE_INDEX if code == SPACE_TOKEN else encode_maps[c] for c in list(code)]   
...
def decode_function(index_list):
    ...
    code = ''.join(code)
    return code
def code_isin_charset(code):
    for i in range(len(code)):
        if code[i] not in charset:
            return False
    return True
